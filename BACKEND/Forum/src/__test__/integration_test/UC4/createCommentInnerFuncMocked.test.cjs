...
jest.mock("../../../utils/hashData.cjs", () => ({
  hashData: jest.fn(() => {
    postIdCounter++;
    return Promise.resolve(`mockedCommentId_${postIdCounter}`);
  }),
  verifyHashedData: jest.fn(),
}));

//now we are testing the base controller function itself, inner functions are mocked
...
describe("Create a comment to a post", () => {
    ...
    expect(res.body.commentId).toMatch("mockedCommentId")
    })
